<?php
	...
	$query_guru = "SELECT * FROM tbl_guru_mapel WHERE id_mata_pelajaran = '$mapel'";
	$sql_guru = $pdo->prepare($query_guru);
	$sql_guru->execute();
	$dt_guru = $sql_guru->fetch();
?>
<!DOCTYPE html>
<html>
<head>
					...
					<div class="row">
						...
						<?php
							while($data = $sql->fetch()){
								...
								?>
										<td><a class="btn btn-warning" href="?page=nilai_mata_pelajaran&aksi=edit_nilai_mata_pelajaran&id=<?php echo $data['nis'];?>&mapel=<?php echo $mapel;?>&nip=<?php echo $dt_guru['nip'];?>">Update Nilai</a></td>
