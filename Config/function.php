<?php
	...
	function kirim_email($email, $nama, $judul, $isipesan)
	{
		...
		try {
		    ...
		    $mail->Host = 'mail.rfpcode.id';  // Specify main and backup SMTP servers
		    $mail->SMTPAuth = true;                               // Enable SMTP authentication
		    $mail->Username = 'akademik@rfpcode.id';                 // SMTP username
		    $mail->Password = 'smkangkasa22';                           // SMTP password
		    $mail->SMTPSecure = 'ssl';                            // Enable TLS encryption, `ssl` also accepted
		    $mail->Port = 465;                                    // TCP port to connect to

			
		    //Recipients
		    $mail->setFrom('akademik@rfpcode.id', 'Akademik E-Learning');
